...
{
    internal class DynamicReconCustomAnalysis : ICustomAnalysis<DynamicReconOptions>
    {
        public IIonData Run(IIonData ionData, IAnalysisTreeNode parentNode, DynamicReconOptions options, IViewBuilder viewBuilder)
		{
			...
			StreamReader Potential = new StreamReader(File.OpenRead(options.InputVoltageCSV));
			string[] array_Vertex = new string[4];
			stcData Data_temp = new stcData();
			while ((line_Vertex = Potential.ReadLine()) != null)
			{
				if (counter_Vertex > 0)
				{
			...
			var voltageChart = viewBuilder.AddChart2D("Voltage",
				"Number of Impacts", "Voltage (V)");
			voltageChart.AddLine(natDat, Voltage, Colors.Red, "Voltage");
			...
			var reconParametersChart = viewBuilder.AddChart2D("Recon Parameters",
				"Number of Impacts", "Field Factor and ICF");
			reconParametersChart.AddLine(natDat, Field_Factor, Colors.Red, "Field Factor");
			reconParametersChart.AddLine(natDat, ICF, Colors.Blue, "ICF");

			var normReconParametersChart = viewBuilder.AddChart2D("Normalized Recon Parameters",
				"Number of Impacts", "Normalized Field Factor and ICF");
			normReconParametersChart.AddLine(natDat, Field_FactorN, Colors.Red, "Normalized Field Factor");
			normReconParametersChart.AddLine(natDat, ICFN, Colors.Blue, "Normalized ICF");
